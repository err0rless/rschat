        ...
        match Command::from_str(&self.input_controller.input) {
            ...
            Ok(Command::Get(item)) => match &item[..] {
                "info" | "name" => {
                    self.input_controller
                        .messages
                        .push("System".to_owned(), format!("Your ID: '{}'", self.state.id));
                }
                _ => {
                    self.input_controller.messages.push(
            ...
            Ok(Command::Register) => {
                let Some(user) = db::user::User::from_stdin().await else {
                    self.input_controller
                        .messages
                        .push("SystemError".to_owned(), "failed to register".to_owned());
            ...
            Ok(Command::Login(login_id)) => {
                    ...
                    .result
                {
                    Ok(_) => {
                        ...
                        self.input_controller
                            .messages
                            .push("System".to_owned(), "Success!".to_owned());
                    }
                    Err(s) => self
                        .input_controller
                        .messages
                        .push("SystemError".to_owned(), format!("Failure: '{}'", s)),
                };
            }
            Ok(Command::Fetch(fetch)) => {
                let item_str = match fetch {
                    ...
                    _ => {
                        self.input_controller
                            .messages
                            .push("SystemError".to_owned(), "Unhandled fetch item".to_owned());
                        return HandleCommandStatus::Continue;
                    }
                };
                ...
                match fetch_res.item.as_str() {
                    "list" => match fetch_res.result {
                        Ok(v) => self.input_controller.messages.push(
                            "System".to_owned(),
                        ...
                        Err(e) => self
                            .input_controller
                            .messages
                            .push("SystemError".to_owned(), e),
                    },
                    unknown => self.input_controller.messages.push(
                        "SystemError".to_owned(),
            ...
            Ok(Command::Goto(channel_name)) => {
                    ...
                    .result
                {
                    ...
                    Err(e) => self.input_controller.messages.push(
                        "SystemError".to_owned(),
            ...
            Ok(Command::Exit) => {
                self.input_controller
                    .messages
                    .push("System".to_owned(), " >> See You Soon << ".to_owned());
                _ = self.outgoing_tx.send(Exit {}.as_json_string()).await;
                return HandleCommandStatus::Exit;
            }
            // Not a command
            Err(ParseCommandError::UnknownCommand(cmd)) => self.input_controller.messages.push(
                "SystemError".to_owned(),
            ...
            Err(e) => self
                .input_controller
                .messages
                .push("SystemError".to_owned(), format!("{:?}", e)),
        }
